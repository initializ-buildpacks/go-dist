	...
	context("when the buildpack is run with pack build", func() {
		...
		context("env vars are used for configuration", func() {
			...
			it("builds with the version from BP_GO_VERSION", func() {
				...
				Eventually(container).Should(Serve(ContainSubstring("go1.25")).OnPort(8080))
				...
				Expect(logs).To(ContainLines(
					...
					"      <unknown>     -> \"\"",
					"",
					MatchRegexp(`    Selected Go version \(using BP_GO_VERSION\): 1\.25\.\d+`),
					"",
					"  Executing build process",
					MatchRegexp(`    Installing Go 1\.25\.\d+`),
